	...
	public class ItemSlot
	{
		...
		public static void Draw(SpriteBatch spriteBatch, Item[] inv, int context, int slot, Vector2 position, Color lightColor = default(Color))
		{
			...
			if (item.type > 0 && item.stack > 0)
			{
				...
				Texture2D value7 = TextureAssets.Item[item.type].get_Value();
				...
				{
					spriteBatch.Draw(value7, position2, rectangle2, item.GetColor(color), 0f, origin, num13 * scale, SpriteEffects.None, 0f);
				}
				...
				if (num14 != -1)
				{
					ChatManager.DrawColorCodedStringWithShadow(spriteBatch, FontAssets.ItemStack.get_Value(), num14.ToString(), position + new Vector2(8f, 30f) * inventoryScale, color, 0f, Vector2.Zero, new Vector2(inventoryScale * 0.8f), -1f, inventoryScale);
				}
				if (context == 13)
				{
				...
				if (context == 13 && item.potion)
				{
					...
					spriteBatch.Draw(TextureAssets.Cd.get_Value(), position3, null, color3, 0f, default(Vector2), num13, SpriteEffects.None, 0f);
				}
				if ((context == 10 || context == 18) && item.expertOnly && !Main.expertMode)
				{
					Vector2 position4 = position + value.Size() * inventoryScale / 2f - TextureAssets.Cd.get_Value().Size() * inventoryScale / 2f;
					Color white = Color.White;
					spriteBatch.Draw(TextureAssets.Cd.get_Value(), position4, null, white, 0f, default(Vector2), num13, SpriteEffects.None, 0f);
				}
			}
			else if (context == 6)
			{
				Texture2D value8 = TextureAssets.Trash.get_Value();
			...
			if (context == 0 && slot < 10)
			{
				...
				ChatManager.DrawColorCodedStringWithShadow(spriteBatch, FontAssets.ItemStack.get_Value(), text2, position + new Vector2(6f, 4 + num16) * inventoryScale, baseColor, 0f, Vector2.Zero, new Vector2(inventoryScale), -1f, inventoryScale);
			}
			if (num != -1)
			{
		...
		public static void DrawSavings(SpriteBatch sb, float shopx, float shopy, bool horizontal = false)
		{
			...
			if (num5 > 0)
			{
				...
				if (num4 > 0)
				{
					sb.Draw(TextureAssets.Item[4076].get_Value(), Utils.CenteredRectangle(new Vector2(shopx + 92f, shopy + 45f), TextureAssets.Item[4076].get_Value().Size() * 0.65f), null, Color.White);
				}
				if (num3 > 0)
				{
					sb.Draw(TextureAssets.Item[3813].get_Value(), Utils.CenteredRectangle(new Vector2(shopx + 92f, shopy + 45f), TextureAssets.Item[3813].get_Value().Size() * 0.65f), null, Color.White);
				}
				if (num2 > 0)
				{
					sb.Draw(TextureAssets.Item[346].get_Value(), Utils.CenteredRectangle(new Vector2(shopx + 80f, shopy + 50f), TextureAssets.Item[346].get_Value().Size() * 0.65f), null, Color.White);
				}
				if (num > 0)
				{
					sb.Draw(TextureAssets.Item[87].get_Value(), Utils.CenteredRectangle(new Vector2(shopx + 70f, shopy + 60f), TextureAssets.Item[87].get_Value().Size() * 0.65f), null, Color.White);
				}
