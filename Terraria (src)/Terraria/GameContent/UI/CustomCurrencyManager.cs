	...
	public class CustomCurrencyManager
	{
		...
		public static void DrawSavings(SpriteBatch sb, int currencyIndex, float shopx, float shopy, bool horizontal = false)
		{
			...
			if (num5 > 0)
			{
				...
				if (num4 > 0)
				{
					sb.Draw(TextureAssets.Item[4076].get_Value(), Utils.CenteredRectangle(new Vector2(shopx + 96f, shopy + 50f), TextureAssets.Item[4076].get_Value().Size() * 0.65f), null, Color.White);
				}
				if (num3 > 0)
				{
					sb.Draw(TextureAssets.Item[3813].get_Value(), Utils.CenteredRectangle(new Vector2(shopx + 80f, shopy + 50f), TextureAssets.Item[3813].get_Value().Size() * 0.65f), null, Color.White);
				}
				if (num2 > 0)
				{
