		...
		public void Draw(List<DrawData> playerDrawData, int drawType, Player drawPlayer, Vector2 Position, Color drawColor, SpriteEffects playerEffect, float shadow)
		{
			...
			case 50:
				if (drawType == 0)
				{
					...
					item = new DrawData(TextureAssets.Extra[207].get_Value(), position, value6, drawColor, num8, origin, scale, spriteEffects, 0);
			...
			case 8:
			{
				...
				for (int i = 0; i < drillMountData2.beams.Length; i++)
				{
					...
					for (int j = 0; j < 2; j++)
					{
						...
						float y = v.Length();
						item = new DrawData(scale: new Vector2(2f, y), texture: TextureAssets.MagicPixel.get_Value(), position: vector6 + Position, sourceRect: value7, color: color2, rotation: num11 - (float)Math.PI / 2f, origin: Vector2.Zero, effect: SpriteEffects.None, inactiveLayerDepth: 0);
