...
namespace Terraria.Graphics.Renderers
{
	public class FlameParticle : ABasicParticle
	{
		...
		public override void Draw(ref ParticleRendererSettings settings, SpriteBatch spritebatch)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				DrawData drawData = new DrawData(position: vector + new Vector2(Utils.RandomInt(ref seed, -2, 3), Utils.RandomInt(ref seed, -2, 3)) * Scale, texture: _texture.get_Value(), sourceRect: _frame, color: color, rotation: Rotation, origin: _origin, scale: Scale, effect: SpriteEffects.None, inactiveLayerDepth: 0);
